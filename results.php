...
// Eigene Ergebnis-Relation: Durchschnitt als Punktzahl von max 10
$sumAll   = calcSum($skala_map['_gesamt'], $ct);
$countAll = count($skala_map['_gesamt']);
$avgNorm  = $countAll>0 ? ($sumAll / ($countAll * itemMax($ct))) * 10 : 0;
$displayErgebnis = number_format($avgNorm, 1, ',', '') . ' von 10';

// Motivations-Text
$motivation  = "🎉 Neugier wecken: Bin gespannt, wie dein Ergebnis ausfällt! ";
$motivation .= "🔍 Eigennutzen: Je mehr mitmachen, desto präzisere Normwerte – auch für dich. ";
$motivation .= "🏆 Spieltrieb: Fordere deine Freunde heraus! ";
$motivation .= $shareUrl;
$shareText = rawurlencode($motivation);
?>
<!doctype html>
<html lang="de">
<head>
...
<div class="container py-4" style="max-width:820px;">
  ...
  <div class="card mb-4"><div class="card-body">
    ...
    <?php if ($participants < NORM_THRESHOLD): ?>
      <p>Noch <strong><?=NORM_THRESHOLD - $participants?></strong> Teilnahmen bis zu aussagekräftigen Normwerten (mindestens <?=NORM_THRESHOLD?>).</p>
    <?php else: ?>
      <p>Ausreichend Daten für Normwerte (≥<?=NORM_THRESHOLD?>).</p>
    <?php endif; ?>
    <?php if ($participants < CRONBACH_THRESHOLD): ?>
      <p>Noch <strong><?=CRONBACH_THRESHOLD - $participants?></strong> Teilnahmen bis Cronbach’s Alpha berechnet werden kann (≥<?=CRONBACH_THRESHOLD?>).</p>
    <?php else: ?>
      <p><strong>Cronbach’s Alpha:</strong> wird jetzt berechnet…</p>
    <?php endif; ?>
  </div></div>
