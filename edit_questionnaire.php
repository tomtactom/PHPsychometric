...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['name'])) {
    ...
    // Einfache Validierung ohne verschachtelte Ternaries
    if (!$name) {
        $errors[] = "Name fehlt.";
    }
    if (!$description) {
    ...
    if (!isset($langs[$language])) {
        $errors[] = "Sprache ungültig.";
    }
    if (!isset($choice_types[$choice_type])) {
        $errors[] = "Skalentyp ungültig.";
    }
    if (!$copyright) {
        $errors[] = "Copyright bestätigen.";
    }
    ...
    if (!empty($_POST['author_password'])) {
        ...
        if (strlen($plain) < 8) {
            $errors[] = "Autor-Passwort min. 8 Zeichen.";
        } else {
    ...
    if (empty($clean_items)) {
        $errors[] = "Mindestens ein Item.";
    }
    // Duplikate
    $low = array_map(fn($it)=>mb_strtolower($it['text']), $clean_items);
    if (count($low) !== count(array_unique($low))) {
        $errors[] = "Duplikate in Items.";
    }
    ...
    if (empty($errors)) {
        if ($editing) {
            $fields = $has_results
              ? "name=?,short=?,language=?,description=?,operationalization=?,author_password_hash=?,updated_at=NOW()"
              : "name=?,short=?,language=?,choice_type=?,description=?,operationalization=?,author_password_hash=?,updated_at=NOW()";
            $params = $has_results
...
document.addEventListener('DOMContentLoaded', ()=>{
  ...
  form.addEventListener('submit', ()=>{
    ...
    subOps.querySelectorAll('.sub-op').forEach(ta=>{
      if(ta.value.trim()) data.subscales[ta.dataset.name]=ta.value.trim();
    });
